...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <div className="hidden md:flex md:w-1/2 bg-white border-r border-slate-100 relative items-center justify-center p-20">
         ...
         <div className="relative z-10 space-y-12 max-w-lg">
            ...
            <div className="space-y-6">
               <h1 className="text-2xl md:text-4xl md:text-6xl md:text-8xl font-black text-slate-900 tracking-tighter leading-[0.9]">
            ...
            <div className="space-y-8 pt-10 border-t border-slate-100">
               {[
                  ...
                  <div key={i} className="flex gap-5">
                     <div className="w-12 h-12 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600"><item.icon className="w-6 h-6" /></div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 md:p-20 relative">
         ...
         <Card className="w-full max-w-md rounded-[4rem] border-none bg-white shadow-[0_40px_100px_rgba(0,0,0,0.05)] p-12 space-y-10 border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 p-2 bg-slate-50 rounded-[2rem]">
               <button onClick={() => setIsLogin(true)} className={cn("h-12 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all", isLogin ? "bg-white text-blue-600 shadow-xl" : "text-slate-400")}>Login</button>
               <button onClick={() => setIsLogin(false)} className={cn("h-12 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all", !isLogin ? "bg-white text-blue-600 shadow-xl" : "text-slate-400")}>Signup</button>
            </div>
            ...
            <form onSubmit={handleAuth} className="space-y-6">
                  ...
                  {!isLogin && (
                     <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
                        ...
                        <div className="relative">
                           ...
                           <Input required value={displayName} onChange={e => setDisplayName(e.target.value)} className="h-16 rounded-2xl bg-slate-50 border-none px-14 font-bold" placeholder="Architect Name" />
                        </div>
                     </motion.div>
                  )}
               </AnimatePresence>
               <div className="space-y-3">
                  ...
                  <div className="relative">
                     ...
                     <Input required type="email" value={email} onChange={e => setEmail(e.target.value)} className="h-16 rounded-2xl bg-slate-50 border-none px-14 font-bold" placeholder="architect@resumepro.ai" />
                  </div>
               </div>
               <div className="space-y-3">
                  ...
                  <div className="relative">
                     ...
                     <Input required type="password" value={password} onChange={e => setPassword(e.target.value)} className="h-16 rounded-2xl bg-slate-50 border-none px-14 font-bold" placeholder="••••••••" />
                  </div>
               </div>
               <Button disabled={loading} className="w-full h-16 bg-blue-600 hover:bg-blue-700 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-2xl shadow-blue-600/30 gap-4 mt-6">
            ...
            <div className="grid grid-cols-1 gap-4">
               <Button variant="outline" onClick={() => handleSocialLogin("google")} className="h-14 rounded-2xl border-slate-200 font-black uppercase tracking-widest text-[10px] gap-3 text-slate-900 bg-white hover:bg-slate-50">
               ...
               <Button variant="outline" onClick={() => handleSocialLogin("apple")} className="h-14 rounded-2xl border-slate-200 font-black uppercase tracking-widest text-[10px] gap-3 text-slate-900 bg-white hover:bg-slate-50">
                  <svg viewBox="0 0 384 512" className="w-4 h-4 text-slate-900" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
