      ...
      <SEOHead title="Performance Analytics - ResumePro" description="Review your interview performance." />
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 md:grid-cols-4 gap-6">
            {[
               ...
               <Card key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-[1.5rem] hover:shadow-lg transition-all duration-300 group cursor-pointer hover:-translate-y-1">
                  <div className="flex items-center justify-between mb-4">
                     <div className={cn("p-2.5 rounded-xl transition-colors", stat.bg)}>
                        <stat.icon className={cn("w-4 h-4", stat.color)} />
                     </div>
                     <Badge variant="outline" className="text-[8px] border-slate-200 dark:border-slate-800 text-slate-400 font-bold px-2 py-0.5">+2.4%</Badge>
                  </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-8">
               <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-[2rem] shadow-sm">
               ...
               <div className="space-y-4">
                  <div className="flex items-center gap-2 px-2">
                     ...
                     <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight uppercase">Recent Sessions</h3>
                  </div>
                  
                  {historyList.length === 0 ? (
                     <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 border-dashed p-12 rounded-[2rem] text-center space-y-4">
                        <Target className="w-10 h-10 text-slate-300 mx-auto" />
                        <div>
                           <p className="text-sm font-bold text-slate-900 dark:text-white">No Interview History</p>
                     ...
                     <div className="space-y-4">
                        {historyList.map(session => (
                           <Card key={session.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl hover:border-blue-600/30 transition-all group">
                              <div className="flex flex-col md:flex-row justify-between gap-6">
                                 <div className="space-y-2 flex-1">
                                    <div className="flex items-center gap-3">
                                       <h4 className="text-lg font-black text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">{session.role}</h4>
                                       <Badge variant="outline" className="text-[8px] font-bold uppercase border-slate-200 dark:border-slate-800 text-slate-400">{session.date}</Badge>
                                    </div>
            ...
            <div className="lg:col-span-4 space-y-8">
               <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-[2rem] shadow-sm relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none">
                     <Brain className="w-20 h-20 text-slate-900 dark:text-white" />
                  </div>
