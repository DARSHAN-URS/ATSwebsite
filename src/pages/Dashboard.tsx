  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 text-left pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         {[
           { label: td.statsApplications, value: stats.apps, sub: "Active pipeline tracking", icon: Zap, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-950/20" },
           { label: td.statsInterviews, value: appsByStatus.filter(a => a.status === "interview").length, sub: "Scheduled this week", icon: Star, color: "text-purple-600 dark:text-purple-400", bg: "bg-purple-50 dark:bg-purple-950/20" },
           { label: td.statsResumeScore, value: optimizationScore === 0 ? "0/100" : `${optimizationScore}/100`, sub: "ATS Optimization", icon: FileText, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950/20" },
           { label: td.statsResponseRate, value: `${stats.responseRate}%`, sub: "Current operational rate", icon: TrendingUp, color: "text-orange-600 dark:text-orange-400", bg: "bg-orange-50 dark:bg-orange-950/20" },
         ].map((s, i) => (
           <motion.div 
             key={i} 
           ...
           >
              <Card className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:shadow-md transition-all duration-300 p-6 group cursor-pointer">
                 ...
                 <div className="space-y-1">
                    ...
                    <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{s.label}</p>
         ...
         <div className="lg:col-span-8 space-y-8">
            ...
            <Card className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
               <div className="p-6 border-b border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                     ...
                     <h3 className="text-sm font-bold text-slate-900 dark:text-white">{td.recentActivity}</h3>
                  </div>
                  <Button onClick={() => navigate("/resumes")} variant="ghost" size="sm" className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-450 transition-colors">{td.viewAll}</Button>
               </div>
               <div className="p-0">
                  {loadingContent ? (
                     <div className="p-12 text-center text-slate-400 dark:text-slate-500 font-medium">Synchronizing activity...</div>
                  ) : activities.length === 0 ? (
                     <div className="p-12 text-center text-slate-400 dark:text-slate-500 font-medium italic">No recent operational activity detected.</div>
                  ) : (
                     activities.map((activity, i) => (
                       <div key={i} onClick={() => navigate(activity.type === "app" ? "/job-tracker" : "/resumes")} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-50 dark:border-slate-800/40 last:border-0 group cursor-pointer">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                <p className="text-[13px] font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{activity.title}</p>
                                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{activity.company} • {activity.time}</p>
                             </div>
                          </div>
                          <div className={cn(
            ...
            <div className="space-y-4">
               <div className="flex items-center justify-between">
                  ...
                  <Link to="/job-board" className="text-[10px] font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 hover:underline">{td.viewGrid}</Link>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {loadingContent ? (
                     [1,2].map(i => <div key={i} className="h-32 rounded-2xl bg-white dark:bg-slate-900 animate-pulse border border-slate-100 dark:border-slate-800" />)
                  ) : recommendations.length === 0 ? (
                     <div className="col-span-2 py-12 text-center text-slate-400 dark:text-slate-500 font-medium italic bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">Initialize job search to see recommendations.</div>
                  ) : (
                     recommendations.map((job, i) => (
                       <Card key={i} onClick={() => navigate("/job-board")} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 hover:border-blue-600/30 transition-all group cursor-pointer shadow-sm">
                          <div className="flex justify-between items-start mb-4">
                             <div className="w-10 h-10 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700/50 flex items-center justify-center font-bold text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                               {job.company.charAt(0)}
                             </div>
                             <div className="bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400 px-2 py-1 rounded-lg text-[10px] font-bold">
                                {job.match} {td.match}
                             </div>
                          </div>
                          <div className="space-y-3">
                             <div>
                                <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{job.role}</p>
                                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{job.company}</p>
                             </div>
                             <div className="flex flex-wrap gap-1.5">
                                {job.tags.map(tag => (
                                  <span key={tag} className="px-2 py-0.5 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-300 text-[9px] font-bold uppercase rounded-md">{tag}</span>
                                ))}
                             </div>
                          </div>
                       </Card>
         ...
         <div className="lg:col-span-4 space-y-8">
            ...
            <Card className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
                <div className="flex items-center gap-3">
                   ...
                   <h3 className="text-sm font-bold text-slate-900 dark:text-white">{td.resumeProgress}</h3>
                </div>
                <div className="space-y-6">
                   <div className="relative pt-1">
                      <div className="flex mb-2 items-center justify-between">
                         <div>
                            <span className="text-[10px] font-bold inline-block py-1 px-2 uppercase rounded-full text-blue-600 dark:text-blue-450 bg-blue-50 dark:bg-blue-950/20">
                      ...
                      <div className="overflow-hidden h-1.5 mb-4 text-xs flex rounded bg-slate-100 dark:bg-slate-800">
                   ...
                   <div className="space-y-3">
                     {checklist.map((step, i) => (
                       <div key={i} className="flex items-center justify-between">
                          <span className="text-[11px] font-medium text-slate-500 dark:text-slate-400">{step.label}</span>
                          {step.done ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> : <div className="w-3.5 h-3.5 rounded-full border border-slate-200 dark:border-slate-800" />}
                       </div>
                     ))}
                   </div>
                   <Button onClick={() => navigate("/resumes")} className="w-full h-10 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[11px] font-bold uppercase tracking-widest hover:bg-blue-600 dark:hover:bg-blue-600 dark:hover:text-white transition-all">{t.resumes.gradeResume}</Button>
                </div>
             </Card>
             ...
             <Card className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-6">
                <div className="flex items-center gap-3">
                   ...
                   <h3 className="text-sm font-bold text-slate-900 dark:text-white">{td.outreachTasks}</h3>
                </div>
                <div className="space-y-4">
                   {outreachTasks.length === 0 ? (
                      <div className="text-center text-slate-400 dark:text-slate-500 font-medium py-8 text-xs italic">
                         No active outreach tasks detected.
                      </div>
                    ) : (
                      outreachTasks.map((task, i) => (
                     <div key={i} onClick={() => navigate("/email-outreach")} className="p-3 rounded-2xl border border-slate-50 dark:border-slate-800/40 hover:border-slate-200 dark:hover:border-slate-700 transition-all group cursor-pointer">
                        <p className="text-[11px] font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-tight mb-1">{task.title}</p>
                        <div className="flex items-center justify-between">
                           <span className="text-[9px] text-slate-400 dark:text-slate-500 font-medium">Due {task.due}</span>
                           <span className={cn(
                   ...
                   <Button onClick={() => navigate("/email-outreach")} variant="outline" className="w-full h-10 rounded-xl border-slate-200 dark:border-slate-800 text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">{td.viewTaskMatrix}</Button>
                </div>
             </Card>
          </div>
      </div>
