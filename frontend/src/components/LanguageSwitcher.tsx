...
export default function LanguageSwitcher({ className }: { className?: string }) {
  ...
  return (
    <Select value={locale} onValueChange={(v) => setLocale(v as Locale)}>
      <SelectTrigger aria-label="Select language" className={className || "w-[140px] h-8 text-xs bg-sidebar-accent/80 text-sidebar-foreground border-sidebar-border [&>svg]:text-sidebar-foreground"}>
